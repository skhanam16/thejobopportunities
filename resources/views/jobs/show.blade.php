      ...
      <section class="md:col-span-3">
          ...
          <div class="container mx-auto p-4">
            ...
            <div x-data="{open: false}">
                <button @click="open = true" class="block w-full text-center px-5 py-2.5 shadow-sm rounded border text-base font-medium cursor-pointer text-indigo-700 bg-indigo-200 hover:bg-indigo-300">
                    Apply Now
                </button>
                <div x-cloak x-show="open" class="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50">
                    <div  @click.away="open = false" class="bg-white p-6 rounded-lg shadow-md w-full max-w-md text-sm">
              ...
              </div>{{-- parent div for Apline component --}}
              @else
...
@endif
            ...
            {{-- Bookmark Button --}}
            @guest
            <p class="mt-10 bg-gray-200 text-gray-700 text-sm w-full py-2 px-4 rounded-full text-center">
                <i class="fas fa-info-circle"></i> You must login to bookmark a job
            </p>
        @else
            <!-- Ensure $job is passed and available in the view -->
